...
{
    public partial class RepRecordPuntualidad : System.Web.UI.Page
    {
        ...
        protected void Page_PreInit(object sender, EventArgs e)
        {
            ...
            List<ReportePuntualidad> list = new List<ReportePuntualidad>();
            ...
            using (proxyRep = new ReportesSrvClient())
            {
                try
                {
                    list = proxyRep.ReportePuntualidad(objSession, listEmpleados.ToArray(), intPeriodo, tipo, ref strMsgUsuario).ToList();
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        if (tipo == 1)
                        {
                            Rel.Load(Server.MapPath("~/Rep/CrystalRep/Puntualidad/RepRecordPuntualidad.rpt"));
                            Rel.SetDataSource(tb);
                            Rel.SetParameterValue("FechaPeriodo", fecIni + " - " + fecFin);
                        }
                        else if (tipo == 2)
                        {
                            Rel.Load(Server.MapPath("~/Rep/CrystalRep/Puntualidad/RepRecordPuntualidadVsTardanza.rpt"));
                            Rel.SetDataSource(tb);
                        ...
                        ReportePuntualidad.ReportSource = Rel;
                        ReportePuntualidad.ToolPanelView = CrystalDecisions.Web.ToolPanelViewType.None;
                        ReportePuntualidad.HasToggleGroupTreeButton = false;
                    }
                    else
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.AlmacenarLogError(ex, "RepRecordPuntualidad");
                }
            }
        }
        ...
        public void ExportPDF(ReportDocument Rel, int tipo)
        {
            ...
            if (tipo == 1)
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Record Puntualidad.pdf""");
            }
            else if (tipo == 2)
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Record Puntualidad Gráfico.pdf""");
            }            
            Response.Flush();
            Response.Close();
        }
        ...
        public void ExportExcel(ReportDocument Rel, int tipo)
        {
            ...
            if (tipo == 1)
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Record Puntualidad.xls""");
            }
            else if (tipo == 2)
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Record Puntualidad Gráfico.xls""");
            }        
            Response.Flush();
            Response.Close();
        }
