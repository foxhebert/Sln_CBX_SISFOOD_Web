...
{
    public partial class RepConsumoDetalladoCosto : System.Web.UI.Page
    {
        ...
        protected void Page_PreInit(object sender, EventArgs e)
        {
            ...
            using (proxyRep = new ReportesSrvClient())
            {
                try
                {
                    list = proxyRep.ReporteDiarioComedor(objSession, listEmpleados.ToArray(), bitCosto, filtrojer_ini, filtrojer_fin, ref strMsgUsuario, int.Parse(intTipoServicio), int.Parse(intTipoMenu), int.Parse(intMarcador), bitAtendido).ToList();//modificado 24.09.2021 
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        if (bitCosto)
                        {
                            ...
                            Rel.Load(Server.MapPath("~/Rep/CrystalRep/Comedor/RepConsumoDetalladoCosto.rpt"));
                        }
                        else
                        {

                            Rel.Load(Server.MapPath("~/Rep/CrystalRep/Comedor/RepConsumoDetallado.rpt"));
                        }
                ...
                catch (Exception ex)
                {
                    ...
                    Log.AlmacenarLogError(ex, "RepConsumoDetalladoCosto.aspx.cs");
                }
            }
        }
        ...
        public void ExportPDF(ReportDocument Rel, bool bitCosto)
        {
            ...
            if (bitCosto)
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Consumo Detallado - Costeado.pdf""");
            }
            else
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Consumo Detallado.pdf""");
            }
        ...
        public void ExportExcel(ReportDocument Rel, bool bitCosto)
        {
            ...
            if (bitCosto)
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Consumo Detallado - Costeado.xls""");
            }
            else
            {
                Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Consumo Detallado.xls""");
            }
            Response.Flush();
            Response.Close();
        }
