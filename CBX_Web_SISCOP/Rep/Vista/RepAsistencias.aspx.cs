...
{
    public partial class RepAsistencias : System.Web.UI.Page
    {
        ...
        protected void Page_PreInit(object sender, EventArgs e)
        {
            ...
            int intMarcador = Int32.Parse(Request.QueryString["intMarcador"]);
            ...
            List<ReporteAsistencia> list = new List<ReporteAsistencia>();
            ...
            using (proxyRep = new ReportesSrvClient())
            {
                try
                {
                    list = proxyRep.ReporteAsistencia(objSession, listEmpleados.ToArray(), intMarcador, filtrojer_ini, filtrojer_fin, ref strMsgUsuario).ToList();
                    DataTable tb = ConvertDataTable(list);

                    if (list.Count > 0)
                    {
                        Rel = new ReportDocument();
                        Rel.Load(Server.MapPath("~/Rep/CrystalRep/RepAsistenciasMHorizontal.rpt"));
                        ...
                        Rel.SetParameterValue("bitManual", "texto");
                        ...
                        ReporteAsistenciaId.ReportSource = Rel;
                        ReporteAsistenciaId.ToolPanelView = CrystalDecisions.Web.ToolPanelViewType.None;
                        ReporteAsistenciaId.HasToggleGroupTreeButton = false;
                    }
                    else
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.AlmacenarLogError(ex, "RepAsistencias");
                }
            }
        }
        ...
        public void ExportPDF(ReportDocument Rel)
        {
            ...
            Response.AddHeader("content-disposition", @"attachment;filename=""Reporte de Asistencia.pdf""");
            Response.Flush();
            Response.Close();
        }
        ...
        public void ExportExcel(ReportDocument Rel)
        {
            ...
            Response.BinaryWrite(stream.ReadBytes(Convert.ToInt32(stream.BaseStream.Length)));
            Response.AddHeader("content-disposition", @"attachment;filename=""Reporte de Asistencias.xls""");
            Response.Flush();
            Response.Close();
        }
