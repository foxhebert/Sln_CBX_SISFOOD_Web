...
{
    public partial class RepResOficial : System.Web.UI.Page
    {
        ...
        protected void Page_PreInit(object sender, EventArgs e)
        {
            ...
            string planilla = Request.QueryString["planilla"];
            ...
            List<ReporteOficial> list = new List<ReporteOficial>();
            ...
            using (proxyRep = new ReportesSrvClient())
            {
                try
                {
                    list = proxyRep.ReporteOficial(objSession, listEmpleados.ToArray(), marca, filtrojer_ini, filtrojer_fin, ref strMsgUsuario).ToList();
                    DataTable tb = ConvertDataTable(list);

                    if (list.Count > 0)
                    {
                        Rel = new ReportDocument();
                        Rel.Load(Server.MapPath("~/Rep/CrystalRep/RepResOficial.rpt"));
                        Rel.SetDataSource(tb);
                        Rel.SetParameterValue("Planilla", planilla);
                        ...
                        ReporteOficial.ReportSource = Rel;
                        ReporteOficial.ToolPanelView = CrystalDecisions.Web.ToolPanelViewType.None;
                        ReporteOficial.HasToggleGroupTreeButton = false;
                    }
                    else
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.AlmacenarLogError(ex, "RepResOficial");
                }
            }
        }
        ...
        public void ExportPDF(ReportDocument Rel)
        {
            ...
            Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Oficial.pdf""");
            Response.Flush();
            Response.Close();
        }
        ...
        public void ExportExcel(ReportDocument Rel)
        {
            ...
            Response.BinaryWrite(stream.ReadBytes(Convert.ToInt32(stream.BaseStream.Length)));
            Response.AddHeader("content-disposition", @"attachment;filename=""Reporte Oficial.xls""");
            Response.Flush();
            Response.Close();
        }
