    ...
    public partial class RepConcesionarioTotalCosto : System.Web.UI.Page
    {
        ...
        protected void Page_PreInit(object sender, EventArgs e)
        {
            ...
            using (proxyRep = new ReportesSrvClient())
            {
                try
                {
                    list = proxyRep.ReporteTotalConcesionaria(objSession, listEmpleados.ToArray(), intPeriodo, int.Parse(intConcesionaria), ref strMsgUsuario, int.Parse(intTipoServicio), int.Parse(intTipoMenu), int.Parse(intMarcador), bitAtendido).ToList();//modificado 24.09.2021  
